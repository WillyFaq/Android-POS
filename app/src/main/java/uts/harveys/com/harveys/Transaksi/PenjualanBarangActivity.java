...
public class PenjualanBarangActivity extends AppCompatActivity {
    ...
    private void get_data(String cari){
        String sql = "SELECT * FROM Barang a JOIN Supplier b ON a.ID_SUPPLIER = b.ID_SUPPLIER" +
                " WHERE NAMA_BARANG LIKE '%"+cari+"%' OR JENIS_BARANG LIKE '%"+cari+"%'";
    ...
    protected void onStart() {
        ...
        list_barang.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int i, long id) {
                ...
                final AlertDialog.Builder builder = new AlertDialog.Builder(PenjualanBarangActivity.this);
                            ...
                            if(jml >= stk){
                                ...
                                if(ca>=0){
                                    ...
                                    if(j >= stk){
                                        ...
                                        DbHelper.detailPenjualan.set(ca, det);
                                        finish();
                                    }
                                }else{
                                    Double st = hrg*jml;
                                    ...
                                    det[3] = jmlh;
                                    det[4] = String.valueOf(st);
                                    DbHelper.detailPenjualan.add(det);
                                    finish();
                                }
                            }
    ...
    private int cek_ada(String id){
        for(int i=0;i<DbHelper.detailPenjualan.size();i++){
            String idb = DbHelper.detailPenjualan.get(i)[0];
            if(id.equals(idb)){
                return i;
            }
        }
