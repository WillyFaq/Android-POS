...
import uts.harveys.com.harveys.Adapter.LaporanDetailAdapter;
import uts.harveys.com.harveys.DbHelper;
import uts.harveys.com.harveys.R;

public class LaporanPenjualanDetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_laporan_penjualan_detail);
        ...
        tgl_awal.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                DatePickerDialog dialog = new DatePickerDialog(
                        LaporanPenjualanDetailActivity.this,
    ...
    private void get_data(){
        String sql = "SELECT * FROM Penjualan_detail a JOIN Barang b ON a.ID_BARANG = b.ID_BARANG JOIN Penjualan c ON a.ID_PENJUALAN = c.ID_PENJUALAN";
        if(!tgl_awal.getText().toString().equals("") || !tgl_akhir.getText().toString().equals("")){
            sql += " WHERE TANGGAL_PENJUALAN BETWEEN '"+tgl_awal.getText().toString()+"' AND '"+tgl_akhir.getText().toString()+"' ";
        }
        Log.e("SQL", sql);
        ...
        while (c.moveToNext()){
            ...
            ret[0] = c.getString(c.getColumnIndex("ID_PENJUALAN_DETAIL"));
            ret[1] = c.getString(c.getColumnIndex("ID_PENJUALAN"));
            ret[2] = c.getString(c.getColumnIndex("ID_BARANG"));
            ret[3] = c.getString(c.getColumnIndex("HARGA"));
            ret[4] = c.getString(c.getColumnIndex("JUMLAH"));
            ret[5] = c.getString(c.getColumnIndex("SUBTOTAL"));
            ret[6] = c.getString(c.getColumnIndex("NAMA_BARANG"))+" ("+c.getString(c.getColumnIndex("SIZE"))+")";
            dataLaporan.add(ret);
            Log.e("IDPEN", ret[0]);
            total += Double.parseDouble(ret[5]);
        }
